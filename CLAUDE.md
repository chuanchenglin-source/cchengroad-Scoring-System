...
> [!IMPORTANT]
> **⚠️ 2026-05-27 路線變更 — Supabase 已停止**
>
> 本檔案部分章節屬於**已停止運作的 Supabase 路線**，請忽略：
> - `# 技術架構` / `## 核心 Schema` / `## 資料存取模式` / `## 管理後台` / `## 技術債（demo 階段...）`
> - `## 新機器 / Mac 首次設定`（含 `setup-env.mjs` / `npm run env:dev` 等指令）
> - `## 測試環境 / 正式環境切換`（含 `.clasp.dev.json` / `.clasp.prod.json` 雙環境機制）
>
> **確定方向**：Google Sheets → GAS only。
> **本機環境**：單一 `.clasp.json` 直接指向正式 GAS，沒有 dev/prod 切換機制。
> **不能執行**：`clasp push`（會直接污染正式環境）、`npm run env:*`、`npm run push:*`。
>
> **仍適用**的章節：`# 專案背景`、`# 相關資源`、`# Claude Code 行為指引`、`## 開發流程`、`## 給Claude讀取的資料/ 跨機器同步機制`、`## clasp 使用注意事項`、`# 活動架構`、`# 計分規則`、`# 系統功能需求`、`# 待確認事項`、`# 開發注意事項`。
>
> 最新狀態見 memory `project-direction-gas-only` 與 `reference-companion-systems`。
